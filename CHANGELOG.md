...
* JSON API calls raise `CarunaAuthError` on 401/403 and `CarunaApiError` on other non-OK responses, instead of returning the error body. `get_metering_points()` no longer hides a failed request as an empty list.
